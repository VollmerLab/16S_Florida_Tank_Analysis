...
upset(
  ...
  stripes = c("#F8EAFF", "#FFE4E4", "#E7FFE5", rep(c("gray91", "gray97"), 5))) +
...
upset(
  comp_upset_bac_strat %>% left_join(venn_all_times_and_doses, by = c("asv_id" = "OTU")),
  colnames(comp_upset_bac_strat %>% left_join(venn_all_times_and_doses, by = c("asv_id" = "OTU")) %>%
             select(crasher_t7_strict, crasher_t7, crasher_t3,late_opportunist, continuous_opportunist, early_opportunist, late_pathogen, continuous_pathogen, early_pathogen)), 
...
the_plots <- bacterial_signature_asv %>%  #merged_bacterial_signature_asv %>%
  ...
  mutate(plot = list(
    ggplot(data = plot_info, aes(x = c_time, y = estimate, ymin = conf.low, ymax = conf.high)) +
      ...
      scale_color_manual(aesthetics = "colour3", values = c("#29A5B2", "#F10A0A"), guide = "legend", 
