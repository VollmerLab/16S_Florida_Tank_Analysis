...
exposure_samples <- sample_data(microbiome_raw) %>%
  ...
  filter(time != 'Acerv',
         tank != 'HOMO') %>% 
  mutate(exposure = if_else(time == 'T0', 'F', exposure),
         tank = if_else(time == 'T0', 'Field', tank),
         sample_id_store = sample_id,
         sample_id = str_replace(sample_id, 'REP1_[RS]', 'F_Field')) %>%
  full_join(preprocess_metadata,
          by = join_by(sample_id, time, exposure, tank, genotype)) %>%
  mutate(retain_sample = TRUE,
         sample_id = if_else(tank == 'Field', NA_character_, sample_id)) %>%
  mutate(sample_id = coalesce(sample_id, sample_id_store), .keep = 'unused')
  

homogenate_samples <- sample_data(microbiome_raw) %>%
  as_tibble(rownames = 'sample_id') %>%
  filter((time == 'Acerv' | tank == 'HOMO')) %>%
  mutate(retain_sample = TRUE) %>%
  mutate(time = 'T0',
