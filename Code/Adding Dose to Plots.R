the_plots123 <- bacterial_signature_asv %>%
  ...
  mutate(plot = list(
    ggplot(data = plot_info, aes(x = c_time, y = estimate, ymin = conf.low, ymax = conf.high, 
      ...
      (geom_point(data = (plot_info %>% filter(graph_cat %in% c("H_S", "H_R"))), size = 3, aes(colour2 = graph_cat, pch = graph_cat)) %>%
         rename_geom_aes(new_aes = c("colour" = "colour2"))) +
      (geom_point(data = (plot_info %>% filter(graph_cat == "dose" & exposure == "H")), size = 3, shape = "diamond", col = "green")) +
      (geom_point(data = (plot_info %>% filter(graph_cat == "dose" & exposure == "D")), size = 3, shape = "diamond", col = "purple")) +
      
    
      geom_errorbar(data = (plot_info %>% filter(graph_cat == "dose" & exposure == "H")), width = 0, col = "green") + 
      
      (geom_errorbar(data = (plot_info %>% filter(graph_cat == "dose" & exposure == "D")), width = 0, col = "purple")) +
      ...
      scale_shape_manual(values = c(17, 16, 1, 17, 16), guide = "none") +
